...
Handles checkout process, payment processing with Stripe,
...
def cache_checkout_data(request):
    """
    Cache checkout data in Stripe payment intent.
    Stores cart, save_info preference, and username in the payment intent metadata.
    
    Args:
        request: HTTP request containing payment intent data
    Returns:
        HttpResponse: 200 if successful, 400 if error
    """
    try:
...
def checkout(request):
    ...
    Processes the order form and creates a Stripe payment intent.
    
    On GET: Display checkout form
    On POST: Process payment and create order
    
    Returns:
        Rendered checkout page or redirect to success/failure page
    """
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            try:
                # Verify the payment was successful first
                pid = request.POST.get('client_secret').split('_secret')[0]
...
def checkout_success(request, order_number):
    ...
    Args:
        ...
        order_number: Order number of successful order
    Returns:
